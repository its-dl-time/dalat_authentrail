...
def crawl_videos_raw(args):
    ...
    if not args.dry_run:
        ...
        print(f"python data_pipeline\\src\\data_scraping\\tiktok.py process-videos --raw-file \"{raw_path}\"")
...
def process_videos_raw(args):
    ...
    rejections = []
    max_total_per_place = args.max_seed_videos_total_per_place
    if max_total_per_place is None:
        max_total_per_place = args.max_seed_videos_per_query * QUERIES_PER_PLACE
    print(f"[TIKTOK VIDEOS] Max seed videos per query: {args.max_seed_videos_per_query}")
    print(f"[TIKTOK VIDEOS] Max seed videos total per place: {max_total_per_place}")
    for row in raw_rows:
    ...
    for (place_id, _query), candidates in candidates_by_query.items():
        ...
        remaining = max_total_per_place - place_counts[place_id]
...
def crawl_comments_raw(args):
    ...
    place_ids = parse_csv_arg(args.place_ids)
    if place_ids:
        seeds = seeds[seeds["place_id"].astype(str).isin(place_ids)].copy()
    ...
    if not args.dry_run:
        ...
        print(f"python data_pipeline\\src\\data_scraping\\tiktok.py process-comments --raw-file \"{raw_path}\"")
    if args.process and not args.dry_run:
        args.raw_file = raw_path
        args.latest = False
...
def parse_args():
    ...
    crawl_videos.set_defaults(
        ...
        max_seed_videos_per_query=3,
        max_seed_videos_total_per_place=None,
    )
    ...
    process_videos.add_argument("--max-seed-videos-per-query", type=int, default=3)
    process_videos.add_argument(
        "--max-seed-videos-total-per-place",
        type=int,
        default=None,
        help=(
            "Optional safety cap per place. If omitted, uses "
            "--max-seed-videos-per-query * QUERIES_PER_PLACE."
        ),
    )
    ...
    crawl_comments_parser.add_argument("--place-ids", default="", help="Only crawl seed videos for these place IDs, comma-separated.")
