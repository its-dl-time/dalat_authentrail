...
def crawl_sources(args: argparse.Namespace) -> None:
    ...
    if not args.dry_run:
        ...
        print(f"python src\\innostar\\data_scraping\\facebook.py process-sources --raw-file \"{raw_path}\"")
...
def crawl_posts(args: argparse.Namespace) -> None:
    ...
    if not args.dry_run:
        ...
        print(f"python src\\innostar\\data_scraping\\facebook.py process-posts --raw-file \"{raw_path}\"")
    if args.process and not args.dry_run:
        args.raw_file = raw_path
        args.latest = False
        process_posts(args)
...
def load_comment_targets(args: argparse.Namespace, fb3) -> pd.DataFrame:
    targets = fb3.load_comment_targets(limit=args.limit_posts, include_direct=not args.no_direct_sources)
    place_ids = parse_place_ids(args.place_ids)
...
def crawl_comments(args: argparse.Namespace) -> None:
    ...
    if not args.dry_run:
        ...
        print(f"python src\\innostar\\data_scraping\\facebook.py process-comments --raw-file \"{raw_path}\"")
    if args.process and not args.dry_run:
        args.raw_file = raw_path
        args.latest = False
